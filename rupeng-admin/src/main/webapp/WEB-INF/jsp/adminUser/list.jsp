				...
				<tr>
					...
					<th>操作
						<button class="btn size-M radius"
							onclick="showLayer('添加管理员','<%=ctxPath%>/adminUser/add.do')">
							添加</button>
					</th>
				</tr>
			</thead>
			<tbody>
				<c:forEach items="${adminUsers}" var="adminUser">
					<tr>
						...
						<td>
							<c:if test="${not adminUser.isDisabled}">
								<span class="label label-success radius">已启用</span>
							</c:if>
							<c:if test="${adminUser.isDisabled}">
						...
						<td>
							...
							<button class="btn size-MINI radius"
								onclick="showLayer('重置密码','<%=ctxPath%>/adminUser/resetPassword.do?id=${adminUser.id}')">重置密码</button>
							<button class="btn size-MINI radius"
								onclick="showLayer('分配角色','<%=ctxPath%>/adminUserRole/update.do?adminUserId=${adminUser.id}')">分配角色</button>
